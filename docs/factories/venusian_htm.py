...
from .custom_tagged import ParseError, tag
...
class htm:
    """ Custom htm decorator that acts as a venusian scannable """

    def __init__(self, *args, **kwargs):
        cache_maxsize = kwargs.get('cache_maxsize', 128)
        self.cached_parse = functools.lru_cache(maxsize=cache_maxsize)(htm_parse)

    def __call__(self, wrapped):
        cached_parse = self.cached_parse

        def callback(scanner, name: str, func):
            cf = getattr(scanner, 'component_factory')
